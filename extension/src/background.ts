...
function safeInfo(message: string, data?: any, code?: string) {
...
async function ensureAuthenticated(): Promise<void> {
  try {
    ...
    try {
      ...
      if (success) safeInfo('Auto-login successful', undefined, 'AUTH_OK');
    } catch (error: any) {
...
async function checkConnectionAndProcessQueueWithAuth(): Promise<void> {
  try {
    const authenticated = await isAuthenticated();
    if (!authenticated) {
      try {

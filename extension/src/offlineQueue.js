    ...
    try {
        ...
        for (const pending of pendingUploads) {
            try {
                ...
                const metadataIndex = screenshotMetadata.findIndex((m) => m.stepIndex === pending.stepIndex && m.timestamp === pending.timestamp);
                if (metadataIndex >= 0) {
 ...
 */
export async function addToOfflineQueue(screenshot, key) {
    try {
        const { pendingUploads = [] } = await chrome.storage.local.get('pendingUploads');
        const pending = {
 ...
 */
export async function checkConnectionAndProcessQueue() {
    if (!navigator.onLine) {
